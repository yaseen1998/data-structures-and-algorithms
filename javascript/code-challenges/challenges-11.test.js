...
const count = (target, input) => {
  // Solution code here...
  let count = input.map(item => item.filter(x => x == target).length).reduce((a, b) => a + b, 0);
  return count
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let count = input.map(item => item.filter(x =>typeof(x) === "number"&& x % 5 == 0  ));
  let result = count.map(ele=>ele.map(element=>Math.pow(2,element)))
  return result
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let gender = data.map(item => item.gender === 'female' || item.gender === 'male'? item.name : '')
  return gender.filter(String).join(' and ')
};
